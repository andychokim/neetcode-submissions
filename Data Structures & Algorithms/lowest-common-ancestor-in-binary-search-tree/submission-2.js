...
class Solution {
    ...
    lowestCommonAncestor(root, p, q) {
        ...
        // base cases
            // if null node -> no LCA possible. empty return
        // at each node - comparing root.val to p and q
        ...
        if (root.val < Math.min(p.val, q.val)) return this.lowestCommonAncestor(root.right, p, q);
        return root;
        ...
        // ** "Do I already know the answer?" → base case, return immediately
        // ** "Do I need to decide where to go before recursing?" → pre-order decision
    }
}
